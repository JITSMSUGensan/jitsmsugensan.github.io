...
export function Cell({ chance }: { chance: number }) {
    ...
    return <div className={`size-[20px] duration-500 ease-out transition-all bg-[#042926] dark:bg-[#4FB9AF] ${value > 0.85 ? "scale-100" : "scale-0"}`}></div>
}
...
export function Announcement({ imageSrc, description, ...props }: { imageSrc: string, description: string, props?: any }) {
    return (
        <div className="relative border border-[#B6F0FF] dark:border-[#030D0F] min-h-[591px] max-w-full md:max-w-[450px] p-[12px] group space-y-[12px] cursor-pointer mx-2 xl:mx-0">
            ...
            <p className="flex-1 font-krona text-[16px] text-[#B6F0FF] dark:text-black">{description}</p>
