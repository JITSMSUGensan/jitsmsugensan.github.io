...
import { IntervalArrayContext } from "@/app";
...
export function Cell({ chance }: { chance: number }) {
  ...
  return (
    <div
      className={`size-[20px] duration-500 ease-out transition-all bg-[#042926] dark:bg-[#4FB9AF] ${
        value > 0.85 ? "scale-100" : "scale-0"
      }`}
    ></div>
  );
}
...
export function Grid({ direction = "top" }: { direction?: "top" | "bottom" }) {
  ...
  useEffect(() => {
    const getNumberOfColumns = () => {
      setNumberOfColumns(
        Array.from({ length: _.ceil(window.innerWidth / 20) }, (_, i) => i)
      );
    };
    getNumberOfColumns();
...
export function Announcement({
  ...
  return (
    <div className="relative border border-[#B6F0FF] dark:border-[#030D0F] min-h-[591px] max-w-full md:max-w-[450px] p-[12px] group space-y-[12px] cursor-pointer mx-2 xl:mx-0">
      <img src={imageSrc} className="w-full aspect-auto object-cover" />
      <p className="flex-1 font-krona text-[16px] text-[#B6F0FF] dark:text-black">
        {description}
      </p>
  ...
  return (
    <div className="flex flex-col min-h-screen justify-center z-[50]">
      ...
      <div className="flex flex-col items-center min-h-[588px] bg-[#042926] dark:bg-[#4FB9AF] transition-all ease-in-out space-y-[42px] pt-[42px] pb-[42px]">
        <p className="text-[34px] delay-150 text-[#B6F0FF] dark:text-black font-krona min-h-[77px] border-b border-[#B6F0FF] dark:border-[#030D0F]">
          ANNOUNCEMENTS
        </p>
        <div className="grid lg:grid-cols-3 md:grid-cols-2 lg:[] gap-[10px] lg:gap-[12px] xl:gap-[64px]">
          {children}
        </div>
      </div>
